...
export async function POST(request: NextRequest) {
  try {
    ...
    let recordCount = 0
    const batchSize = 50000 // Reduced batch size for better memory management
    let batch: any[] = []
      ...
      while (true) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (record) {
            ...
            // Insert batch when it reaches the batch size or every 30 seconds
            if (batch.length >= batchSize || (batch.length > 0 && Date.now() - lastDbCommit > 30000)) {
              await insertBatch(tempTableName, batch)
              batch = []
              lastDbCommit = Date.now()
              ...
              if (now - lastProgressUpdate > 10000) { // Update every 10 seconds
                await redis.set(`upload_progress:${sessionId}`, JSON.stringify({
                  ...
                  total: file.size,
                  progress: (totalBytesProcessed * 100 / file.size).toFixed(1),
                  stage: 'processing'
                }))
...
// Optimized batch insert using parameterized query with transaction
async function insertBatch(tableName: string, batch: any[]) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // Split large batches into smaller chunks to avoid parameter limits
    ...
    for (let i = 0; i < values.length; i += chunkSize) {
      ...
      await tx.$executeRawUnsafe(`
        INSERT INTO ${tableName} (
          ...
          $${index * 20 + 16}, $${index * 20 + 17}, $${index * 20 + 18}, $${index * 20 + 19}, $${index * 20 + 20}
        )`).join(', ')}
      `, chunk.flat())
    }
  })
...
async function createIndexes(tableName: string) {
  await prisma.$executeRawUnsafe(`CREATE INDEX idx_${tableName}_date ON ${tableName} (dataDate)`)
  await prisma.$executeRawUnsafe(`CREATE INDEX idx_${tableName}_website ON ${tableName} (website)`)
  await prisma.$executeRawUnsafe(`CREATE INDEX idx_${tableName}_country ON ${tableName} (country)`)
  await prisma.$executeRawUnsafe(`CREATE INDEX idx_${tableName}_composite ON ${tableName} (dataDate, website, country)`)
}