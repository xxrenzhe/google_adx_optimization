...
function mapHeaders(headers: string[]) {
  ...
  headers.forEach((h, idx) => {
    ...
    else if ([ 'clicks','点击','点击数' ].includes(s)) map['clicks']=idx
  ...
  try {
    ...
    const session = await (prisma as any).uploadSession.create({
      data: {
        filename: file.name,
        status: 'uploading',
        tempTableName: `staging_offer_${crypto.randomUUID().replace(/-/g,'')}`,
        fileSize: file.size,
        dataType: 'offer'
      }
    })
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      batch.push({ dataDate: new Date(dateStr), website, revenue, clicks, impressions })
    }
    const size = 1000
...
async function copyImportOffer(sessionId: string, file: File): Promise<number> {
  ...
  try {
    ...
    await client.query('CREATE TEMP TABLE IF NOT EXISTS staging_offer (LIKE public.offer_revenue INCLUDING DEFAULTS);')

    const cols = ['dataDate','website','revenue','clicks','impressions']
    const copySql = `COPY staging_offer(${cols.join(',')}) FROM STDIN WITH (FORMAT csv)`
    const dbStream = client.query(copyFrom(copySql))

    let skipped = true // 已经弹出首行
    let processed = 0
    ...
    for (const line of lines) {
      ...
      const row = [ q(get('date','')), q(get('website','Unknown')), nOrEmpty(getNum('revenue',0)), nOrEmpty(parseInt(getBigStr('clicks')||'0')), nOrEmpty(parseInt(getBigStr('impressions')||'0')) ].join(',') + '\n'
      await write(row)
      processed++
    ...
    const upsertSql = `
      INSERT INTO public.offer_revenue (${cols.join(',')})
      SELECT ${cols.join(',')} FROM staging_offer
