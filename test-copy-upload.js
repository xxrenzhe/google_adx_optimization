...
// Test script for comparing COPY command vs batch insert
const fs = require('fs');
...
console.log('🚀 Testing COPY command upload with 35M CSV file');
    ...
    res.on('end', () => {
        ...
        try {
            ...
            if (result.sessionId) {
                ...
                fs.writeFileSync('last-session-copy.json', JSON.stringify({
                ...
                console.log('\n💾 Session info saved to last-session-copy.json');
                
                // Compare with previous results if available
                if (fs.existsSync('last-session.json')) {
                    const previous = JSON.parse(fs.readFileSync('last-session.json', 'utf8'));
                    console.log('\n📈 Performance Comparison:');
                    console.log(`Previous method: ${previous.recordCount?.toLocaleString()} records in ${previous.duration.toFixed(2)}s`);
                    console.log(`COPY method:    ${result.recordCount?.toLocaleString()} records in ${duration.toFixed(2)}s`);
                    const improvement = ((previous.duration - duration) / previous.duration * 100).toFixed(1);
