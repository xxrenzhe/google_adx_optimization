...
export default function DataTableOptimized({ refreshTrigger }: DataTableOptimizedProps) {
  ...
  const columns = useMemo(() => [
    { key: 'dataDate', label: '日期', format: (value: any) => value ? new Date(value).toLocaleDateString() : '-', width: '120px' },
    ...
    { key: 'requests', label: '请求数', format: (value: any) => value?.toLocaleString() || '0', width: '100px' },
    { key: 'impressions', label: '展示数', format: (value: any) => value?.toLocaleString() || '0', width: '100px' },
    { key: 'clicks', label: '点击数', format: (value: any) => value?.toLocaleString() || '0', width: '100px' },
    { key: 'ctr', label: '点击率', format: (value: any) => value ? `${(value * 100).toFixed(2)}%` : '0%', width: '100px' },
    { key: 'ecpm', label: 'eCPM', format: (value: any) => value ? `$${value.toFixed(2)}` : '$0.00', width: '100px' },
    { key: 'revenue', label: '收入', format: (value: any) => value ? `$${value.toFixed(2)}` : '$0.00', width: '100px' }
  ], [])
                  ...
                  >
                    {columns.map((column) => (
                      <td 
                        ...
                        style={{ width: column.width }}
                      >
                        {column.format ? column.format(row[column.key as keyof AdReport]) : (row[column.key as keyof AdReport] || '-')}
                      </td>
                    ))}
                  </tr>
                )
